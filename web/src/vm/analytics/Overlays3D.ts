...
export class Overlays3D {
  ...
  rebuildAnalytics(analytics: AnalyticsResult): void {
    ...
    if (this.store.overlays.heatmap) {
    ...
    if (this.store.overlays.traffic) {
    ...
    if (this.store.overlays.adjacency) {
  ...
  rebuildStockSignals(anchor: AnchorLookup): void {
    ...
    this.pulses = [];
    if (!this.store.overlays.stock) return;
